...
local function TeleportPedsToPlayer(ply, typ)
	if not ply then return end if ply==localplayer then H = localplayer:get_heading()*50 else H=0 end
	pos2=ply:get_position() sleep(0.1) pos1=ply:get_position() disX=(pos1.x-pos2.x) disY=(pos1.y-pos2.y) disZ=(pos1.z-pos2.z)
	if not typ or typ==1 then for ped in replayinterface.get_peds() do if ped and ped:get_pedtype()>3 then
		if not ped:is_in_vehicle() then ped:set_position(ply:get_position()+vector3(2*disX,2*disY,disZ)+H) end end end
...
local function DirToZ(ply)
	poz1=player.get_player_ped():get_position() poz2=ply:get_position() sdH=math.sqrt((poz2.y-poz1.y)^2+(poz2.z-poz1.z)^2)
	if poz2.y>poz1.y and poz2.z>poz1.z then Qdr=1 elseif poz2.y<poz1.y and poz2.z>poz1.z then Qdr=2 elseif
	poz2.y<poz1.y and poz2.z<poz1.z then Qdr=3 else Qdr=4 end
	if Qdr==1 or Qdr==3 then sdA=math.sqrt((poz2.y-poz1.y)^2)else sdA=math.sqrt((poz2.z-poz1.z)^2)end
	Agl=math.acos(sdA/sdH)if Qdr>1 then Rad2=Agl+(Qdr-2)*1.57 else Rad2=Agl+4.71 end if Rad2>3.14 then Rad2=Rad2-6.28 end
	print(tostring(Qdr)..","..tostring(sdA)..","..tostring(sdH))
	print(tostring(Rad2))
	print("been ere") DAT_Z=Rad2
	return Rad2
	
end
local DAT_X,DAT_Z=0,0
local function TurnTo(ply)
	if not player.get_player_ped(selectedplayer) then return end
	pos1=player.get_player_ped():get_position() pos2=ply:get_position() sH=math.sqrt((pos2.x-pos1.x)^2+(pos2.y-pos1.y)^2) hi=pos2.z-pos1.z
	Dist=math.sqrt(((pos2.x-pos1.x)^2)+((pos2.y-pos1.y)^2)+((pos2.z-pos1.z)^2))
	...
	Ang=math.acos(sA/sH)if Qd>1 then Rad=Ang+(Qd-2)*1.57 else Rad=Ang+4.71 end if Rad>3.14 then Rad=Rad-6.28 end
	P=math.acos(sH/Dist)if hi<0 then P=-P end DAT_X=Rad DAT_Z=P
	if localplayer:is_in_vehicle() then VRot=localplayer:get_current_vehicle():get_rotation()
		localplayer:get_current_vehicle():set_rotation(Rad,VRot.y,P)else PRot=localplayer:get_rotation() 
		localplayer:set_rotation(Rad,PRot.y,P) end
end

local playerlist=nil
local function BuildList()
	playerlist:clear()
	--[[playerlist:add_bare_item("", function() if localplayer:is_in_vehicle()then R=localplayer:get_current_vehicle():get_rotation()else R=localplayer:get_rotation()end
			return string.format("%.4f",R.x)..","..string.format("%.4f",R.y)..","..string.format("%.4f",R.z) end,
		function() menu.send_key_press(keyNum0)end,
		function()if localplayer:is_in_vehicle()then R=localplayer:get_current_vehicle():get_rotation() localplayer:get_current_vehicle():set_rotation(R.x,R.y,R.z+0.5)
		else R=localplayer:get_rotation() localplayer:set_rotation(R.x,R.y,R.z-0.5)end end,
		function()if localplayer:is_in_vehicle()then R=localplayer:get_current_vehicle():get_rotation() localplayer:get_current_vehicle():set_rotation(R.x,R.y,R.z+0.5)
		else R=localplayer:get_rotation() localplayer:set_rotation(R.x,R.y,R.z+0.5)end end)
	playerlist:add_bare_item("", function() return DAT_X..","..localplayer:get_rotation().y..","..DAT_Z end, null, null, null)]]--

	playerlist:add_toggle("[!]TriggerBot(test) ",function()if globals.get_int(CrVOy)==1 then return true end return false end,function(v)
		if v and selectedplayer>=0 then globals.set_int(CrVOx,selectedplayer)globals.set_int(CrVOy,1)menu.send_key_press(156) else globals.set_int(CrVOy,0)end
	end)
	playerlist:add_bare_item("---AppleVegas's Player List, "..player.get_number_of_players().." Players---", function()
