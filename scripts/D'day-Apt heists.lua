...
local function movePed(ps)
	for ped in replayinterface.get_peds() do if ped:get_pedtype()>3 then if not ped:is_in_vehicle() then if ps==1 then
...
local DRGWR={"Designated Driver[2]","FatalIncursion[3]","Uncontrolled Substance[4]","Make War not Love[5]","Off the Rails[6]"}
DRGWR[0]="Welcome to the Troupe[1]"
menu.add_array_item("First Dose>", DRGWR, function() return stats.get_int("MP"..mpx().."_XM22_CURRENT") end, function(v)
	stats.set_int("MP"..mpx().."_XM22_CURRENT",v)
end)